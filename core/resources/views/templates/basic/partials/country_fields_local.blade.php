<div class="{{ $class }}">
        ...
        <span class="sending-parent">
            <select autocomplete="off" class="sending-countries country-picker" name="sending_country" required>
                @foreach ($sendingCountries as $sendingCountry)
                    <option style="background-color:#F7B787" value="{{ $sendingCountry->id }}"
    ...
    @if ($showLimit)
        <a href="javascript:void(0)" class="showLimit xsm-text text-muted  text-decoration-underline"> <i
                class="la la-info-circle"></i> @lang('Limit')</a>
...
<div class="{{ $class }} d-none">
    ...
    <div class="input-group">
        ...
        <span class="recipient-parent">
            <select class="form-control form--control country-picker recipient-countries" id="recipient-country"
